...
static const uint8_t DES_IP[64] = { 58, 50, 42, 34, 26, 18, 10, 2,
...
static const uint8_t DES_EX[64] = { 32, 1, 2, 3, 4, 5,
							4, 5, 6, 7, 8, 9,
							8, 9, 10, 11, 12, 13,
							12, 13, 14, 15, 16, 17,
							16, 17, 18, 19, 20, 21,
...
static const uint8_t DES_P[64] = { 16, 7, 20, 21,
						   29, 12, 28, 17,
						   1, 15, 23, 26,
						   5, 18, 31, 10,
						   2, 8, 24, 14,
...
static const uint8_t DES_INVIP[64] = { 40, 8, 48, 16, 56, 24, 64, 32,
...
static const uint8_t DES_S_BOX[8][4][16] = { {  {14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7},
...
static const uint8_t DES_PC1_l[28] = { 57, 49, 41, 33, 25, 17, 9,
							   1, 58, 50, 42, 34, 26, 18,
							   10, 2, 59, 51, 43, 35, 27,
							   19, 11, 3, 60, 52, 44, 36 };

static const uint8_t DES_PC1_r[28] = { 63, 55, 47, 39, 31, 23, 15,
							   7, 62, 54, 46, 38, 30, 22,
							   14, 6, 61, 53, 45, 37, 29,
							   21, 13, 5, 28, 20, 12, 4 };
...
static const uint8_t DES_PC2[56] = { 14, 17, 11, 24, 1, 5,
							 3, 28, 15, 6, 21, 10,
							 23, 19, 12, 4, 26, 8,
							 16, 7, 27, 20, 13, 2,
							 41, 52, 31, 37, 47, 55,
...
std::string DESEncryption::run(const std::string& DATA) {
	if (!keyset) {
...
DESEncryption::DESEncryption(const std::string& KEY)
{
	setkey(KEY);
}

void DESEncryption::setkey(const std::string& KEY) {
	if (keyset) {
...
std::string DESEncryption::encrypt(const std::string& DATA) {
	return run(DATA);
}

std::string DESEncryption::decrypt(const std::string& DATA) {
...
size_t DESEncryption::blocksize() const {
	return 64;
}
