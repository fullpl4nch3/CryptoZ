...
#define SPOOKY_SEED1 0x123456789abcdef
#define SPOOKY_SEED2 0xfedcba987654321
...
public:
	SpookyHash()
	{
		state = new spooky_state;

		state->m_length = 0;
		state->m_remainder = 0;
		state->m_state[0] = SPOOKY_SEED1;
		state->m_state[1] = SPOOKY_SEED2;
	}
	~SpookyHash()
	{
		if (state)
...
private:
	...
	static VOID spooky_shorthash(LPBYTE message, size_t length, uint64_t* hash1, uint64_t* hash2)
	{
	...
	static VOID spooky_update(struct spooky_state* state, const BYTE* message, DWORD length)
	{
	...
	)
	{
		...
		if (state->m_length < SC_BUFSIZE)
		{
			spooky_shorthash((LPBYTE)state->m_data, state->m_length, hash1, hash2);
			return;
		}
	...
	static VOID spooky_hash128(LPBYTE message, size_t length, uint64_t* hash1, uint64_t* hash2)
	{
		uint64_t h0, h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11;
...
public:
	static ULONGLONG SpookyDigest(const BYTE* pData, DWORD dwDataSize)
	{
		ULONGLONG hash1 = SPOOKY_SEED1;
		ULONGLONG hash2 = SPOOKY_SEED2;

		spooky_hash128((LPBYTE)pData, dwDataSize, &hash1, &hash2);
